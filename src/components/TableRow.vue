<template>
    <tr>
        <td>{{ title }}</td>
        <td class="flex -mt-1 items-center" @click="input ? showed = true : null">
            <div class="px-2 py-1" v-if="!showed" >
                {{ type }}
            </div>
            <input v-if="showed"
                   v-focus
                   ...
                   :type="input" class="w-full px-2 py-1"
            >
            <svg v-if="!showed && input" class="stroke-mainRed-400 ml-3" width="18" height="24" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const props = defineProps(['showed', 'title', 'type', 'input'])
